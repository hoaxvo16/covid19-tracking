export default async function getCovidData() {
   ...
   for (const element of data.Countries) {
      ...
      let flag;
      if (arr.length > 1) {
         let formatted = formatCountryName(arr);
         flag = `https://cdn.countryflags.com/thumbs/${formatted}/flag-square-500.png`;
      } else {
         if (element.Country === 'Timor-Leste')
            flag = `https://cdn.countryflags.com/thumbs/east-timor/flag-square-500.png`;
         else
            flag = `https://cdn.countryflags.com/thumbs/${element.Country.toLowerCase()}/flag-square-500.png`;
      }
      let temp = {
         id: i,
         flag: flag,
